...
class PerformanceDashboardView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if emp:
            # 查找我的评估记录
            ctx['my_evaluations'] = PerformanceEvaluation.objects.filter(emp=emp).select_related('cycle')
            refresh_metrics_for_queryset(ctx['my_evaluations'], save=True)

        cycles: Sequence[PerformanceCycle] = cast(Sequence[PerformanceCycle], ctx.get('cycles') or [])
        for cycle in cycles:
...
class PerformanceCycleCreateView(LoginRequiredMixin, PerformanceAdminRequiredMixin, CreateView):
    # 仅CFO/绩效部门/管理员可用
    ...
    def form_valid(self, form):
        ...
        employees = Employee.objects.filter(org=cycle.org) if getattr(cycle, 'org_id', None) else Employee.objects.all()
        ...
        messages.success(self.request, f"绩效周期已创建，并初始化了 {created} 名员工的评估记录。")
        return response


class PerformanceCycleDeleteView(LoginRequiredMixin, PerformanceAdminRequiredMixin, DeleteView):
    """删除绩效周期。"""
    model = PerformanceCycle
    success_url = reverse_lazy('performance:dashboard')
...
class PerformanceEvaluationManageListView(LoginRequiredMixin, PerformanceAdminRequiredMixin, ListView):
    """绩效部门统一管理：评估列表。"""
    ...
    def get_queryset(self):
        qs = PerformanceEvaluation.objects.select_related('cycle', 'emp').order_by('-cycle__start_time', 'emp__emp_name')
        refresh_metrics_for_queryset(qs, save=True)
        return qs


class PerformanceEvaluationManageUpdateView(LoginRequiredMixin, PerformanceAdminRequiredMixin, UpdateView):
...
class PerformanceCycleStatusUpdateView(LoginRequiredMixin, PerformanceAdminRequiredMixin, View):
    ...
    def post(self, request, pk):
        ...
        messages.success(request, f'周期状态已更新为“{valid_status[new_status]}”，相关评估状态已同步。')
        return redirect('performance:dashboard')


class PerformanceSearchView(LoginRequiredMixin, PerformanceAdminRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if keyword:
            kw = f"%{keyword.lower()}%"
            clauses.append("(LOWER(emp.emp_name) LIKE %s OR LOWER(auth.username) LIKE %s)")
            params.extend([kw, kw])
        ...
        for row in rows:
            ...
            row['attendance_rate_display'] = None if att is None else f"{float(att) * 100:.2f}%"
            row['leave_rate_display'] = None if leave is None else f"{float(leave) * 100:.2f}%"
