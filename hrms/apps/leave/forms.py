...
class LeaveApplyForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            'leave_type': forms.Select(attrs={'class': 'form-select block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50'}),
            'reason': forms.Textarea(attrs={'class': 'form-textarea block w-full mt-1 border-gray-300 rounded-md shadow-sm', 'rows': 3}),
