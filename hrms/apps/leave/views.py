...
from utils.sql_scope import build_org_tree_cte, get_user_scope, normalize_str, parse_iso_datetime_local, uniq
    ...
    def get_context_data(self, **kwargs):
        ...
        if is_performance_admin(self.request.user):
            perf_qs = PerformanceEvaluation.objects.filter(
                evaluation_status='hr_audit'
            ).select_related('emp', 'cycle').order_by('cycle__start_time', 'emp__emp_name')
...
class LeaveDetailView(LoginRequiredMixin, DetailView):
    ...
    def _resolve_approver(self, leave):
        ...
        if leave.update_by:
            approver = user_model.objects.filter(pk=leave.update_by).select_related('employee').first()
            if approver:
...
class LeaveOrgSqlSearchView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        if keyword:
            kw = f"%{keyword.lower()}%"
            clauses.append("(LOWER(v.emp_name) LIKE %s OR LOWER(v.emp_id) LIKE %s OR LOWER(COALESCE(v.reason,'')) LIKE %s)")
