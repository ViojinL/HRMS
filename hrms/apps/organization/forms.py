...
class OrganizationCreateForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["org_type"].choices = [
            choice
            for choice in self.fields["org_type"].choices
            if choice[0] != "company"
        ]
        for field in self.fields.values():
            if getattr(field.widget, "input_type", None) != "date":
                ...
                field.widget.attrs["class"] = (
                    f"{css} w-full border-gray-300 rounded-lg shadow-sm focus:border-primary focus:ring focus:ring-primary/20 transition-colors".strip()
                )
