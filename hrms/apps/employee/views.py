...
from utils.sql_scope import build_org_tree_cte, get_user_scope, normalize_str, parse_iso_date, uniq
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            for row_idx, row in enumerate(sheet.iter_rows(min_row=2, values_only=True), start=2):
                ...
                try:
                    name, emp_id, id_card, birth_date, phone, email, org_code, position, hire_date, gender = row[:10]
                except ValueError:
...
class EmployeeTemplateDownloadView(LoginRequiredMixin, View):
    def get(self, request):
        ...
        ws = wb.active
        ws.title = "员工导入模板"
        headers = ['姓名', '工号', '身份证号', '出生日期', '手机号', '邮箱', '部门编码', '岗位', '入职日期', '性别(male/female)']
        ws.append(headers)
        ws.append(['张三', 'IMP9001', '110101199001011234', '1990-01-01', '13800000000', 'zhangsan@test.com', 'HR', '人事专员', '2024-01-01', 'male'])
        output = BytesIO()
        wb.save(output)
        output.seek(0)
...
class EmployeeOrgSqlSearchView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        if keyword:
            kw = f"%{keyword.lower()}%"
            clauses.append("(LOWER(v.emp_name) LIKE %s OR LOWER(v.emp_id) LIKE %s OR LOWER(v.email) LIKE %s)")
            params.extend([kw, kw, kw])

        if manager_keyword:
            kw = f"%{manager_keyword.lower()}%"
            clauses.append("(LOWER(COALESCE(v.manager_emp_name,'')) LIKE %s OR LOWER(COALESCE(v.manager_emp_code,'')) LIKE %s)")
            params.extend([kw, kw])
...
class HROnboardingView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def _context_data(self, form=None):
        ...
        pending_onboardings = Employee.objects.filter(emp_status='probation').order_by('-hire_date')[:5]
    ...
    def get(self, request):
        form = self.form_class(initial={'employment_type': 'full_time', 'emp_status': 'probation'})
    ...
    def post(self, request):
        ...
        if action == 'approve_onboarding' and self._is_hr_director():
            ...
            if emp_pk:
                Employee.objects.filter(pk=emp_pk).update(emp_status='active', update_by=request.user.username)
        ...
        self.request.session['last_onboarding_creds'] = {'username': emp_id, 'password': password}
        messages.success(request, f'已为 {emp.emp_name} 创建账号 {emp_id}，初始密码已通过提示展示。')
        return redirect(self.success_url)
