...
class DashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["user_display_name"] = (
            emp.emp_name if emp and emp.emp_name else user.username
        )
        ...
        if not emp:
            if user.is_superuser:
                ...
                context["active_users"] = AuthUser.objects.filter(
                    is_active=True
                ).count()

                # 获取全站最近动态
                context["recent_activities"] = AuditLog.objects.all().order_by(
                    "-oper_time"
                )[:10]
                return context
            else:
        ...
        pending_approvals = LeaveApply.objects.filter(
            emp__manager_emp=emp, apply_status="pending"
        ).count()
