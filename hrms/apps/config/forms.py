    ...
    class Meta:
        ...
        widgets = {
            "code": forms.TextInput(
                attrs={"placeholder": "如 annual", "autocomplete": "off"}
            ),
            "name": forms.TextInput(attrs={"placeholder": "如 年假"}),
            "description": forms.Textarea(
                attrs={"rows": 3, "placeholder": "简短说明该理由的使用场景"}
            ),
    ...
    def __init__(self, *args, **kwargs):
        ...
        for name, field in self.fields.items():
            ...
            if name == "status":
                field.widget.attrs.update(
                    {
                        "class": "w-full rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm text-gray-700"
                    }
                )
                continue
